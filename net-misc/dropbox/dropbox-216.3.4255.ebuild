...
SRC_URI="https://www.github.com/funtoo/dropbox-python-setup/tarball/1.1 -> dropbox-python-setup-1.1.tar.gz
	amd64? ( https://edge.dropboxstatic.com/dbx-releng/client/dropbox-lnx.x86_64-216.3.4255.tar.gz -> dropbox-lnx.x86_64-216.3.4255.tar.gz )
...
pkg_setup() {
	...
	if use gnome; then
		S="$WORKDIR/nautilus-dropbox-2024.04.17"
	else
		S="$WORKDIR"
	fi
...
src_unpack() {
	if use gnome; then
		unpack nautilus-dropbox-2024.04.17.tar.bz2
	fi
